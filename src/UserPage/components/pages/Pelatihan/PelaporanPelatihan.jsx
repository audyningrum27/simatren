...
const PelaporanPelatihan = () => {
  ...
  useEffect(() => {
    const fetchPelatihanProses = async () => {
      ...
      try {
        const response = await axios.get(`https://be-simatren.riset-d3rpla.com/api/data_pelatihan/pelatihan-proses/${idPegawai}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`https://be-simatren.riset-d3rpla.com/api/data_pelatihan/upload-bukti/${selectedPelatihan}`, formData, {
        headers: {
      ...
      const { data } = await axios.get(`https://be-simatren.riset-d3rpla.com/api/data_pegawai/pegawai/profil/${idPegawai}`);
      const notifikasiData = {
        id_pegawai: idPegawai,
        message: `${data.nama_pegawai} mengirimkan pelaporan pelatihan.`
      };

      await axios.post('https://be-simatren.riset-d3rpla.com/api/data_notifikasi/notifikasi-admin/pelatihan', notifikasiData);
      setShowPopup(true);
    } catch (error) {
