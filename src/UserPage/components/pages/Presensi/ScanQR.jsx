...
const ScanQR = () => {
  ...
  useEffect(() => {
    ...
    if (idPegawai) {
      fetch(`https://be-simatren.riset-d3rpla.com/api/data_presensi/presensi/status/${idPegawai}`)
  ...
  const handleScanMasuk = (data) => {
    ...
    if (data && idPegawai) {
      ...
      fetch(`https://be-simatren.riset-d3rpla.com/api/data_presensi/save-presensi/${idPegawai}`, {
        method: 'POST',
        headers: {
